...
def normalize_per_sample_channels(time_series, eps=1e-8):
    """
    Per-sample, per-channel z-score normalization along time dimension.

    Args:
        time_series: np.ndarray, shape (N, 2, 128)
        eps: small constant to avoid division by zero
    """
    mean = np.mean(time_series, axis=2, keepdims=True)
    std = np.std(time_series, axis=2, keepdims=True)
    return (time_series - mean) / (std + eps)


def _prepare_time_series_data(data):
    """Convert raw table rows into normalized IMU time series and metadata."""
    ...
    time_series = normalize_per_sample_channels(time_series).astype(np.float32)

    meta = {
        "labels": data["label"].astype(str).values,
        "sub_labels": data["sub_label"].astype(str).values,
        "users": data["user"].astype(str).values,
        "file_ids": (
            data["file_id"].astype(str).values
            if "file_id" in data.columns
            else np.array([f"sample_{i}" for i in range(len(data))], dtype=str)
        ),
    }
    return time_series, meta


def load_time_series_with_meta(time_domain_file, target_label=None):
    """
    Load time-domain IMU data along with label/user/file metadata.

    Returns:
        tuple: (time_series, meta)
            time_series: np.ndarray (N, 2, 128)
            meta: dict with labels/sub_labels/users/file_ids
    """
    data = _read_table(time_domain_file)
    if target_label is not None:
        data = data[data["label"] == target_label].reset_index(drop=True)
    return _prepare_time_series_data(data)
...
def load_time_series(time_domain_file, target_label=None):
    ...
    time_series, meta = _prepare_time_series_data(data)
    return time_series, meta["labels"], meta["users"], meta["file_ids"]
